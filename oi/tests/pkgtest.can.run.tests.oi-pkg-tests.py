...
def canPassTest():
	...
	tests.out("command|pkgtest " + root + "/pkgtest.passing.test.py")
	tests.assertOn(tests.hasOutput("PASSED A passing test"))

def canFailTest():
	root = os.path.dirname(__file__)
	tests.out("command|pkgtest " + root + "/pkgtest.failed.test.py")
	tests.assertOn(tests.hasOutput("FAILED A failing test"))

def canMarkTestAsInconclusuve():
	root = os.path.dirname(__file__)
	tests.out("command|pkgtest " + root + "/pkgtest.inconclusive.test.py")
